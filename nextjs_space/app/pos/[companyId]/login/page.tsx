"use client";

import { useState, useEffect, useRef } from "react";
import { useParams, useRouter } from "next/navigation";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { NumericKeypad } from "@/components/numeric-keypad";
import { Modal } from "@/components/modal";
import { LoadingSpinner } from "@/components/loading-spinner";
import { usePOS } from "@/lib/pos-context";
import { formatCurrency } from "@/lib/helpers";
import { User, Barcode, ArrowLeft, ScanLine, DollarSign, ShieldCheck, CheckCircle } from "lucide-react";
import { motion } from "framer-motion";

interface Employee {
  id: string;
  name: string;
  role: string;
  barcode: string;
}

interface Shift {
  id: string;
  openingBalance: number | null;
  status: string;
}

export default function POSLoginPage() {
  const params = useParams();
  const router = useRouter();
  const companyId = params?.companyId as string;
  const { setCompanyId, setEmployee, setShiftId } = usePOS();
  
  const [barcode, setBarcode] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const [lastInputTime, setLastInputTime] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);
  
  // Opening balance state
  const [showOpeningBalance, setShowOpeningBalance] = useState(false);
  const [openingBalanceAmount, setOpeningBalanceAmount] = useState("");
  const [loggedInEmployee, setLoggedInEmployee] = useState<Employee | null>(null);
  const [currentShift, setCurrentShift] = useState<Shift | null>(null);
  
  // Manager approval state
  const [showManagerApproval, setShowManagerApproval] = useState(false);
  const [managerBarcode, setManagerBarcode] = useState("");
  const [managerError, setManagerError] = useState("");
  const [approving, setApproving] = useState(false);
  const managerInputRef = useRef<HTMLInputElement>(null);
  
  useEffect(() => {
    setCompanyId(companyId);
  }, [companyId, setCompanyId]);
  
  // Auto-focus barcode input
  useEffect(() => {
    if (inputRef.current && !loading && !showOpeningBalance && !showManagerApproval) {
      inputRef.current.focus();
    }
  }, [loading, showOpeningBalance, showManagerApproval]);
  
  // Auto-focus manager input
  useEffect(() => {
    if (managerInputRef.current && showManagerApproval) {
      setTimeout(() => managerInputRef.current?.focus(), 100);
    }
  }, [showManagerApproval]);
  
  // Refocus on click anywhere
  useEffect(() => {
    const handleClick = () => {
      if (!showOpeningBalance && !showManagerApproval) {
        if (inputRef.current && !loading) {
          inputRef.current.focus();
        }
      } else if (showManagerApproval && managerInputRef.current) {
        managerInputRef.current.focus();
      }
    };
    document.addEventListener("click", handleClick);
    return () => document.removeEventListener("click", handleClick);
  }, [loading, showOpeningBalance, showManagerApproval]);
  
  const handleLogin = async (scannedBarcode?: string) => {
    const barcodeToUse = scannedBarcode || barcode;
    
    if (!barcodeToUse.trim()) {
      setError("Please scan your employee barcode");
      return;
    }
    
    setLoading(true);
    setError("");
    
    try {
      const res = await fetch("/api/employees/verify-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyId, barcode: barcodeToUse }),
      });
      
      if (!res.ok) {
        setError("Invalid employee barcode");
        setBarcode("");
        return;
      }
      
      const employee = await res.json();
      setLoggedInEmployee(employee);
      setEmployee(employee);
      
      // Create or get existing shift
      const shiftRes = await fetch(`/api/companies/${companyId}/shifts`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ employeeId: employee.id }),
      });
      
      if (shiftRes.ok) {
        const shift = await shiftRes.json();
        setCurrentShift(shift);
        setShiftId(shift.id);
        
        // Check if opening balance needs to be set
        if (shift.openingBalance === null || shift.openingBalance === undefined) {
          // Show opening balance modal
          setShowOpeningBalance(true);
          setBarcode("");
        } else {
          // Already has opening balance, go directly to menu
          router.push(`/pos/${companyId}/menu`);
        }
      } else {
        router.push(`/pos/${companyId}/menu`);
      }
    } catch (err) {
      console.error("Login error:", err);
      setError("Failed to login. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  
  const handleOpeningBalanceSubmit = () => {
    const amount = parseFloat(openingBalanceAmount);
    if (isNaN(amount) || amount < 0) {
      return;
    }
    // Proceed to manager approval
    setShowOpeningBalance(false);
    setShowManagerApproval(true);
  };
  
  const handleManagerApproval = async (scannedBarcode?: string) => {
    const barcodeToUse = scannedBarcode || managerBarcode;
    
    if (!barcodeToUse.trim()) {
      setManagerError("Please scan manager barcode");
      return;
    }
    
    setApproving(true);
    setManagerError("");
    
    try {
      // Verify manager barcode
      const res = await fetch("/api/employees/verify-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyId, barcode: barcodeToUse }),
      });
      
      if (!res.ok) {
        setManagerError("Invalid barcode");
        setManagerBarcode("");
        return;
      }
      
      const manager = await res.json();
      
      // Check if manager
      if (manager.role !== "manager") {
        setManagerError("Manager authorization required");
        setManagerBarcode("");
        return;
      }
      
      // Update shift with opening balance
      const amount = parseFloat(openingBalanceAmount) || 0;
      const updateRes = await fetch(`/api/shifts/${currentShift?.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          openingBalance: amount,
        }),
      });
      
      if (updateRes.ok) {
        // Success - proceed to menu
        setShowManagerApproval(false);
        router.push(`/pos/${companyId}/menu`);
      } else {
        setManagerError("Failed to save opening balance");
      }
    } catch (err) {
      console.error("Manager approval error:", err);
      setManagerError("Failed to verify manager. Please try again.");
    } finally {
      setApproving(false);
    }
  };
  
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    const now = Date.now();
    
    setBarcode(value);
    setError("");
    
    // Detect fast input (scanner) vs manual typing
    if (value.length > barcode.length + 1 || (now - lastInputTime < 50 && value.length > 5)) {
      // Auto-submit on scanner input
      if (value.startsWith("EMP-") && value.length >= 9) {
        setTimeout(() => handleLogin(value), 100);
      }
    }
    
    setLastInputTime(now);
  };
  
  const handleManagerInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setManagerBarcode(value);
    setManagerError("");
    
    // Auto-submit on scanner input
    if (value.startsWith("EMP-") && value.length >= 9) {
      setTimeout(() => handleManagerApproval(value), 100);
    }
  };
  
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && barcode.trim()) {
      handleLogin();
    }
  };
  
  const handleManagerKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && managerBarcode.trim()) {
      handleManagerApproval();
    }
  };
  
  // Handle paste (scanner often pastes)
  const handlePaste = (e: React.ClipboardEvent) => {
    const pastedText = e.clipboardData.getData("text").trim();
    if (pastedText.startsWith("EMP-")) {
      e.preventDefault();
      setBarcode(pastedText);
      setTimeout(() => handleLogin(pastedText), 100);
    }
  };
  
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <Button
        variant="ghost"
        onClick={() => router.push("/pos")}
        className="absolute top-4 left-4 text-gray-400 hover:text-white"
      >
        <ArrowLeft className="h-4 w-4 mr-2" />
        Back
      </Button>
      
      {/* Main login UI */}
      {!showOpeningBalance && !showManagerApproval && (
        <>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-center mb-8"
          >
            <div className="w-20 h-20 bg-pos-card rounded-full flex items-center justify-center mx-auto mb-4 border border-pos-border">
              <User className="h-10 w-10 text-green-400" />
            </div>
            <h1 className="text-2xl font-bold">Employee Login</h1>
            <p className="text-gray-400 mt-2">Scan your employee barcode to clock in</p>
          </motion.div>
          
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
            className="w-full max-w-md"
          >
            <div className="mb-6">
              <div className="relative">
                <div className="p-8 bg-pos-card border-2 border-dashed border-pos-border rounded-xl flex flex-col items-center gap-4">
                  <motion.div
                    animate={{ opacity: [0.5, 1, 0.5] }}
                    transition={{ duration: 2, repeat: Infinity }}
                    className="text-green-400"
                  >
                    <ScanLine className="h-16 w-16" />
                  </motion.div>
                  <p className="text-gray-400 text-sm">Waiting for barcode scan...</p>
                  
                  <Input
                    ref={inputRef}
                    value={barcode}
                    onChange={handleInputChange}
                    onKeyDown={handleKeyDown}
                    onPaste={handlePaste}
                    placeholder="Scan or enter employee barcode"
                    className="bg-gray-800 border-gray-600 text-white text-center font-mono text-lg tracking-wider"
                    autoComplete="off"
                  />
                  
                  {barcode && (
                    <div className="flex items-center gap-2 text-green-400">
                      <Barcode className="h-5 w-5" />
                      <span className="font-mono">{barcode}</span>
                    </div>
                  )}
                </div>
              </div>
              
              {error && (
                <p className="text-red-400 text-center text-sm mt-4">{error}</p>
              )}
            </div>
            
            <Button
              onClick={() => handleLogin()}
              disabled={loading || !barcode.trim()}
              className="w-full h-14 text-lg bg-green-600 hover:bg-green-700 disabled:bg-gray-700"
            >
              {loading ? <LoadingSpinner size="sm" /> : "Clock In"}
            </Button>
            
            <p className="text-center text-gray-500 text-xs mt-4">
              Don&apos;t have your barcode? Contact a manager to print a new one.
            </p>
          </motion.div>
        </>
      )}
      
      {/* Opening Balance Step */}
      {showOpeningBalance && (
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="w-full max-w-md"
        >
          <div className="text-center mb-6">
            <div className="w-16 h-16 bg-green-600/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <DollarSign className="h-8 w-8 text-green-400" />
            </div>
            <h1 className="text-2xl font-bold">Set Opening Balance</h1>
            <p className="text-gray-400 mt-2">Welcome, {loggedInEmployee?.name}!</p>
            <p className="text-gray-500 text-sm">Enter your cash drawer opening balance</p>
          </div>
          
          <div className="bg-pos-card border border-pos-border rounded-xl p-6">
            <div className="text-center py-4 mb-4">
              <span className="text-4xl font-mono text-green-400">
                ${openingBalanceAmount || "0.00"}
              </span>
            </div>
            
            <NumericKeypad
              onKeyPress={(key) => setOpeningBalanceAmount((prev) => prev + key)}
              onClear={() => setOpeningBalanceAmount("")}
              onBackspace={() => setOpeningBalanceAmount((prev) => prev.slice(0, -1))}
              showDecimal
            />
            
            <Button
              className="w-full h-14 text-lg mt-4 bg-green-600 hover:bg-green-700"
              onClick={handleOpeningBalanceSubmit}
              disabled={!openingBalanceAmount}
            >
              Continue to Manager Approval
            </Button>
            
            <Button
              variant="ghost"
              className="w-full mt-2 text-gray-400"
              onClick={() => {
                setOpeningBalanceAmount("0");
                handleOpeningBalanceSubmit();
              }}
            >
              Skip (Set as $0.00)
            </Button>
          </div>
        </motion.div>
      )}
      
      {/* Manager Approval Step */}
      {showManagerApproval && (
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="w-full max-w-md"
        >
          <div className="text-center mb-6">
            <div className="w-16 h-16 bg-blue-600/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <ShieldCheck className="h-8 w-8 text-blue-400" />
            </div>
            <h1 className="text-2xl font-bold">Manager Approval Required</h1>
            <p className="text-gray-400 mt-2">Manager must verify opening balance</p>
          </div>
          
          <div className="bg-pos-card border border-pos-border rounded-xl p-6">
            <div className="bg-gray-800/50 rounded-lg p-4 mb-6">
              <div className="flex justify-between items-center mb-2">
                <span className="text-gray-400">Employee</span>
                <span className="font-medium">{loggedInEmployee?.name}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400">Opening Balance</span>
                <span className="font-bold text-green-400 text-xl">
                  {formatCurrency(parseFloat(openingBalanceAmount) || 0)}
                </span>
              </div>
            </div>
            
            <div className="p-6 bg-gray-800 border-2 border-dashed border-blue-500/50 rounded-xl flex flex-col items-center gap-4">
              <motion.div
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ duration: 2, repeat: Infinity }}
                className="text-blue-400"
              >
                <ScanLine className="h-12 w-12" />
              </motion.div>
              <p className="text-gray-400 text-sm">Scan manager barcode to approve</p>
              
              <Input
                ref={managerInputRef}
                value={managerBarcode}
                onChange={handleManagerInputChange}
                onKeyDown={handleManagerKeyDown}
                placeholder="Scan manager barcode"
                className="bg-gray-900 border-gray-600 text-white text-center font-mono"
                autoComplete="off"
              />
              
              {managerBarcode && (
                <div className="flex items-center gap-2 text-blue-400">
                  <Barcode className="h-5 w-5" />
                  <span className="font-mono">{managerBarcode}</span>
                </div>
              )}
            </div>
            
            {managerError && (
              <p className="text-red-400 text-center text-sm mt-4">{managerError}</p>
            )}
            
            <Button
              className="w-full h-14 text-lg mt-4 bg-blue-600 hover:bg-blue-700"
              onClick={() => handleManagerApproval()}
              disabled={approving || !managerBarcode.trim()}
            >
              {approving ? <LoadingSpinner size="sm" /> : "Verify & Start Shift"}
            </Button>
            
            <Button
              variant="ghost"
              className="w-full mt-2 text-gray-400"
              onClick={() => {
                setShowManagerApproval(false);
                setShowOpeningBalance(true);
                setManagerBarcode("");
                setManagerError("");
              }}
            >
              Back to Edit Balance
            </Button>
          </div>
        </motion.div>
      )}
    </div>
  );
}
